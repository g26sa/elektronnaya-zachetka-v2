  ...
  const [institution, student] = await Promise.all([
    ...
    prisma.student.findUnique({
      ...
      include: {
        ...
        stateExams: { include: { chair: true, chairGek: true }, orderBy: { date: "asc" } },
      },
    }),
  ]);
  ...
  return {
    institution: institution
      ? {
          ...
          departmentHeadName: institution.departmentHeadName,
          departmentHeadTitle: institution.departmentHeadTitle,
    ...
    courseWorks: student.courseWorks.map((c) => ({
      ...
      topic: c.topic,
      grade: c.grade ?? "",
      date: formatDate(c.date ?? c.assignedAt),
    ...
    stateExams: student.stateExams.map((e) => ({
      ...
      grade: e.grade ?? "",
      chair: e.chairGek?.fullName ?? e.chair?.fullName ?? "",
