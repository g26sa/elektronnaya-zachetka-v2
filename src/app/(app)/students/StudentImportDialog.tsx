...
export function StudentImportDialog() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.missingGroup) {
          setMissingGroup(data.missingGroup);
        } else {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset(); }}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Импорт студентов из Excel</DialogTitle>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {missingGroup && (
              <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800 flex gap-2">
                ...
                <span>
                  Группа <strong>«{missingGroup}»</strong> не найдена.
                  Сначала добавьте её во вкладке{" "}
                  <a href="/groups" className="underline font-medium">Группы</a>.
                </span>
              </div>
            )}
