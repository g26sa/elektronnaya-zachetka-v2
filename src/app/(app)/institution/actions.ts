...
export async function saveInstitution(input: unknown) {
  ...
  if (before) {
    result = await prisma.institution.update({
      where: { id: before.id },
      data: {
        name: d.name, shortName: d.shortName ?? null, address: d.address ?? null,
        ogrn: d.ogrn ?? null, inn: d.inn ?? null, city: d.city ?? null,
        headName: d.headName ?? null, headTitle: d.headTitle ?? null,
      },
    });
  } else {
    result = await prisma.institution.create({
      data: {
        name: d.name, shortName: d.shortName ?? null, address: d.address ?? null,
        ogrn: d.ogrn ?? null, inn: d.inn ?? null, city: d.city ?? null,
        headName: d.headName ?? null, headTitle: d.headTitle ?? null,
